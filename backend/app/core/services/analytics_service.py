...
class AnalyticsService:
    ...
    async def get_monthly_summary(
        ...
        """Get the expense summary for a specific month, scoped to user."""
    ...
    async def get_category_breakdown(
        ...
        for summary in summaries:
            # Get top 5 transactions for this category, scoped to user
            ...
            breakdown.append(
                {
                    ...
                    "percentage": round(
                        (summary.total_amount / total * 100) if total > 0 else 0,
                        1,
                    ),
    ...
    async def get_trends(
        self, num_months: int = 6, user_id: uuid.UUID = None
    ) -> dict:
        """Get multi-month expense trend data for chart visualization."""
        # Get all available months for this user
        result = await self.db.execute(
            select(
                MonthlySummary.year,
    ...
    async def get_dashboard_summary(self, user_id: uuid.UUID) -> dict:
        ...
        # Find the latest month for this user
        result = await self.db.execute(
        ...
        summary = await self.get_monthly_summary(latest.year, latest.month, user_id)
        trends = await self.get_trends(6, user_id)
